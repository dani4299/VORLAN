...
export const WallpaperSheet = ({ anchorRef, onClose }) => {
  ...
  return createPortal(
    <div
      ref={panelRef}
      className="glass-strong animate-sheet-in fixed z-[100] w-72 rounded-[24px] p-5"
    ...
    >
      ...
      <div className="flex p-1 rounded-full chip mb-4">
        {[{ id: 'dark', label: 'Dark', icon: Moon }, { id: 'light', label: 'Light', icon: Sun }].map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-full text-xs font-medium transition-all duration-150 ${tab === t.id ? 'bg-[var(--accent)] text-white' : 'text-[var(--ink-muted)] hover:text-[var(--ink)]'}`}
          >
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 rounded-full py-2.5 text-sm font-semibold glass text-[var(--ink)] hover:brightness-125 active:scale-[0.97] transition-all duration-150"
      >
