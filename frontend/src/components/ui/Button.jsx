 ...
 */
export const IconButton = React.forwardRef(({ label, title, className = '', children, ...props }, ref) => {
  ...
  return (
    <button
      ...
      aria-label={name}
      title={name}
      className={`inline-flex items-center justify-center rounded-[var(--radius-md)] w-9 h-9 transition-colors duration-150 text-[var(--ink-muted)] hover:text-[var(--ink)] hover:bg-[var(--overlay-3)] ${className}`}
      {...props}
    >
      {children}
    </button>
